...
foreach ($rows as $r) {
	...
	if (count($parts) !== 4 || $parts[0] !== 'assets') {
		continue;
	}
	...
	if (!isset($loaded_dirs[$dir_key])) {
		...
		$loaded_dirs[$dir_key] = is_file($json_path) ? (json_decode(file_get_contents($json_path), true) ?: []) : [];
	}
	$labels = $loaded_dirs[$dir_key];
	$label_map[$src] =
		$labels[$filename] ?? ucwords(str_replace(['-', '_'], ' ', pathinfo($filename, PATHINFO_FILENAME)));
}
...
<html lang="en">
  ...
  <head>
    ...
    <meta name="description" content="View the stage plot &ldquo;<?= esc(
    	$plot['title_staplot'],
    ...
    <div class="wrapper">
      <main class="shared-plot-main">
        ...
        <div class="shared-plot-body<?= !empty($channels) || $details ? ' has-inputs' : '' ?>">
          ...
          <div class="stage-plot-canvas" id="shared-canvas">
